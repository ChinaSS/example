 ...
 * @example
 ...
 <pre>form = {{user | data}}</pre>
 <pre>master = {{master | data}}</pre>
 </div>
...
function $HttpProvider() {
  var JSON_START = /^\s*(\[|\{[^\{])/,
      JSON_END = /[\}\]]\s*$/,
      PROTECTION_PREFIX = /^\)\]\}',?\n/,
      CONTENT_TYPE_APPLICATION_JSON = {'Content-Type': 'application/data;charset=utf-8'};
  ...
  var defaults = this.defaults = {
    ...
    transformResponse: [function(data) {
      if (isString(data)) {
        // strip data vulnerability protection prefix
    ...
    headers: {
      common: {
        'Accept': 'application/data, text/plain, */*'
      },
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *
     * - [JSON vulnerability](http://haacked.com/archive/2008/11/20/anatomy-of-a-subtle-data-vulnerability.aspx)
     ...
     *
     * A [JSON vulnerability](http://haacked.com/archive/2008/11/20/anatomy-of-a-subtle-data-vulnerability.aspx)
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      if (responseType) {
        try {
          ...
          // WebKit added support for the data responseType value on 09/03/2013
          // https://bugs.webkit.org/show_bug.cgi?id=73648. Versions of Safari prior to 7 are
          // known to throw when setting the value "data" as the response type. Other older
          // browsers implementing the responseType
          //
          // The data response type can be ignored if not supported, because JSON payloads are
...
Parser.prototype = {
  ...
  // This is used with data array declaration
 ...
 *     var self = this;
 *     $http.get("test_data.data", {cache: $templateCache}).success(function(userComments) {
